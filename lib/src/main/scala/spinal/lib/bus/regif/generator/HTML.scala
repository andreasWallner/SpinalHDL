...
class HTML(intf: BusIf) {
  ...
  def write(writer: Writer): Unit = {
    writeHeader(writer)
    intf.registers().map(writeRegister(_, writer))
    writeFooter(writer)
  }

  def writeRegister(register: RegInst, writer: Writer): Unit = {
    register.checkLast

    val fieldCnt = register.getFields.size

    writer.write(s"""
        |          <tr class="reg" align="left">
        |            <td align="center" rowspan="${fieldCnt}">0x${register.addr.toHexString}</td>
        ...
        |            <td align="center" rowspan="${fieldCnt}">${register.busif.busDataWidth}</td>
        """.stripMargin)
  ...
  def writeField(field: Field, writer: Writer) = {
    val reserved = if (field.accType == AccessType.NA) "reserved" else ""
    writer.write(s"""
    |          <td class="${reserved}" >${Section(field.section)}</td>
    |          <td class="${reserved}" >${field.name}</td>
    |          <td class="${reserved}" align="center">${field.accType}</td>
    |          <td class="${reserved}" align="right">${formatResetValue(field.resetValue, widthOf(field.hardbit))}</td>
